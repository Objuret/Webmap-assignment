...
// Webmap-assignment/static/js/main.js

document.addEventListener('DOMContentLoaded', function() {
    // 1. Initialize Leaflet Map
    const map = L.map('map').setView([59.3293, 18.0686], 10); // Center on Stockholm
    ...
    // Define base layers for Leaflet's layer control
    const baseLayers = {
        "OpenStreetMap": osmLayer,
        "Esri World Imagery": esriWorldImagery
    };
    const overlayLayers = {}; // No overlays yet

    // Add Leaflet's built-in layer control (your "map-layer button")
    L.control.layers(baseLayers, overlayLayers).addTo(map);

    // 2. Initialize Leaflet Sidebar v2
    const sidebar = L.control.sidebar({
        container: 'sidebar',
        position: 'right'
    }).addTo(map);

    // Invalidate map size when sidebar opens/closes to prevent display issues
    sidebar.on('content', function () { map.invalidateSize(); });
    sidebar.on('hidden', function () { map.invalidateSize(); });
    sidebar.on('shown', function () { map.invalidateSize(); });

    // Initially open the sidebar (optional)
    sidebar.open(''); 
});